...
def default_payload(symbol: str, date_mt5: str) -> Dict[str, Any]:
    """
    Daily audit payload schema (safe default).
    """
    return {
        "schema_version": 1,
        "symbol": symbol,
...
def build_start_root_payload(day_payload: Dict[str, Any]) -> Dict[str, Any]:
    """
    ROOT start file should be small and stable.
    Only include what other runners need.
    """
    return {
        "schema_version": day_payload.get("schema_version", 1),
        "symbol": day_payload.get("symbol"),
        "date_mt5": day_payload.get("date_mt5"),
        "tz": day_payload.get("tz", {}),
        "timestamps": day_payload.get("timestamps", {}),
        "start": day_payload.get("start", {}),
    }